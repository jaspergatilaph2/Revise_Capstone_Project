...
@section('content')
    ...
    <div class="d-grid gap-2 d-md-flex justify-content-md-center">
        ...
        <!-- Under Review Button -->
        @foreach ($user->electricalPlans ?? [] as $permit)
    <form action="{{ route('review.proposal.under-review-electrical', $permit->id) }}" method="POST" class="w-100 w-md-auto">
        @csrf
...
@endforeach
        ...
        <!-- Approve Button -->
        @foreach ( $user->ElectricalPlans ?? [] as $permit )
        <form action="{{ route('review.proposal.approve-electrical', $permit->id) }}" method="POST" class="w-100 w-md-auto">
        @csrf
        ...
        <!-- Delete Button -->
         @foreach ( $user->ElectricalPlans ?? [] as $permit )
         <form action="{{ route('review.proposal.delete-electrical', $permit->id) }}" method="POST" class="w-100 w-md-auto" onsubmit="return confirm('Are you sure you want to delete this permit?');">
            @csrf
            @method('DELETE')
