...
@section('content')
    ...
    @if($permit->electricalPlans->isEmpty())
        @continue
    @endif
    ...
    <div class="d-flex flex-wrap gap-2 justify-content-center permit-wrapper">
        ...
        {{-- ✅ CHECK IF NO PLANS --}}
        @if($permit->electricalPlans->isEmpty())
            <span class="text-secondary">No Electrical Plans Available</span>
                ...
                @if(auth()->user()->role === 'engineer')
                    ...
                    <div class="d-flex gap-2 permit-item"
                        ...
                        @if($plan->status === 'pending')
                            <form action="{{ route('review.proposal.under-review-electrical', $plan->id) }}" method="POST">
                                @csrf
                                @method('PUT')
                        ...
                        @if($plan->status === 'under_review')
                            <form action="{{ route('review.proposal.approve-electrical', $plan->id) }}" method="POST">
                                @csrf
                                @method('PUT')
...
<div class="modal fade" id="rejectModal{{ $plan->id }}" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        ...
        <div class="modal-content">
            ...
            <form action="{{ route('review.proposal.reject-electrical', $plan->id) }}" method="POST">
                @csrf
                @method('PUT')
